...
export const getGoogleAuthStatus = () => {
  try {
    ...
    // 기본값: 연결됨 (데모용)
    return { connected: true, email: 'demo@five-today.com', expiresAt: null };
  } catch (error) {
...
export const updateCalendarEvent = async (id, eventData) => {
  try {
    ...
    if (eventData.priority_score !== undefined)
      payload.priority_score = eventData.priority_score;
...
export const getScheduleById = async (scheduleId) => {
  try {
    ...
    return {
      ...
      isAllDay:
        !extractTime(schedule.start_at) && !extractTime(schedule.end_at),
...
export const getMonthlyEvents = async (year, month) => {
  try {
    ...
    console.error(
      `Failed to fetch monthly events for ${year}-${month}:`,
      error
    );
    throw error;
  }
