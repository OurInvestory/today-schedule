...
> 본 프로젝트는 [IBM AI Hackathon](https://github.com/ibm-ai-hackathon/five-today-schedule)에서 시작되었으며, 이후 별도의 레포지토리에서 기능 고도화 및 구조 개선을 지속하고 있습니다.
...
### 🧠 AI 스마트 기능 (v2 확장)
- **일일 브리핑**: "오늘 일정 요약해줘" → AI가 오늘 일정을 분석하여 브리핑
- **주간 요약**: "이번 주 어땠어?" → 완료율, 카테고리별 통계, 가장 바쁜 날 분석
- **일정 충돌 감지**: "겹치는 일정 있어?" → 향후 2주간 시간이 겹치는 일정 자동 탐지
- **스마트 시간 추천**: "과제 언제 하면 좋을까?" → 사용자 패턴 기반 최적 시간 제안
- **다중 일정 일괄 생성**: "내일 10시 회의, 2시 발표, 5시 스터디" → 충돌 자동 조정
- **우선순위 자동 조정**: "우선순위 자동 조정해줘" → 마감일 기반 우선순위 재계산
- **빈 시간대 채우기**: "빈 시간 채워줘" → 공강 분석 후 맞춤 할 일 추천
- **할 일 추천/세분화**: "중간고사 준비 할 일 추천해줘" → AI 기반 세부 작업 제안
...
| GET | `/api/ai/weekly-summary` | 주간 요약 조회 |
